# YOLO Frame Extraction and Analysis
# This script extracts all frames from videos using ffmpeg and then analyzes each frame with YOLO
...
HUMAN_FRAMES_DIR = "./human_detection"  # Directory to save frames with human detection
LOG_FILE = "./processed_videos.txt"  # Log file to track processed videos

# Create necessary directories
os.makedirs(FRAMES_DIR, exist_ok=True)
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(HUMAN_FRAMES_DIR, exist_ok=True)
...
def detect_humans(image_path):
    ...
    for result in results:
        for box in result.boxes:
            ...
            # Class 0 is person in COCO dataset
            if class_id == 0 and confidence > CONFIDENCE_THRESHOLD:
                # Save the frame to the human_detection directory
                dest_path = os.path.join(HUMAN_FRAMES_DIR, os.path.basename(image_path))
...
def create_clips_from_frames(video_path, frames_dir):
    ...
    for i, frame_path in enumerate(tqdm(frames, desc="Analyzing frames")):
        has_person = detect_humans(frame_path)
